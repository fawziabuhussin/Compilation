 ...
 *)
 ...
 module Reader : READER = struct
   ...
   and nt_vector str =
     ...
     let nt5 = pack nt4 (fun (_, (sexprs, _)) -> ScmVector sexprs) in
     
     let nt_empty = caten nt1 nt2 in
     
     let nt_empty = pack nt_empty (fun _ -> ScmVector []) in
     
     let parse = disj nt_empty nt5 in
     let parse = make_skipped_star parse in
     parse str
   
   
   
   (* TODO : -> ( nt_sexpr )  || (nt_sexpr1 . nt_sexpr2) *)
   and nt_list str =
 ...
 let rec scm_list_to_list = function
   ...
   | ScmNil -> []
   | _ -> raise (X_not_yet_implemented "This is not proper list");;
 ...
 module Tag_Parser : TAG_PARSER = struct
   ...
   let rec macro_expand_qq = function
     ...
     | ScmPair (ScmPair (ScmSymbol "unquote",
                         ScmPair (car, ScmNil)),
                cdr) ->
        let cdr = macro_expand_qq cdr in (* ,@b*)
   ...
   let rec macro_expand_cond_ribs ribs =
     match ribs with
     (* (else (h x y) (g x)) -> (begin (h x y) (g x))*)
     | ScmPair (ScmPair (ScmSymbol "else", body), rest) -> (ScmPair (ScmSymbol "begin", body))
     (* ((h? x) => (p q))*)
     | ScmPair (ScmPair (q, ScmPair (ScmSymbol "=>", body)), ScmNil) -> 
        (ScmPair (ScmSymbol "let", ScmPair (
                                       ScmPair (ScmPair (ScmSymbol "value", ScmPair (q, ScmNil)), (* let ((value (h? x))*)
                                                ScmPair (ScmPair (ScmSymbol "f", ScmPair (ScmPair (ScmSymbol "lambda", 
                                                                                                   ScmPair (ScmNil, 
                                                                                                            ScmPair (ScmPair (ScmSymbol "begin", body), 
                                                                                                                     ScmNil))), 
                                                                                          ScmNil)), 
                                                         ScmNil)), 
                                       ScmPair (ScmPair (ScmSymbol "if", 
                                                         ScmPair (ScmSymbol "value", (* if value*)
                                                                  ScmPair (ScmPair (ScmPair (ScmSymbol "f", ScmNil), (* (f) value*) 
                                                                                    ScmPair (ScmSymbol "value", ScmNil)), 
                                                                           ScmPair (ScmPair (ScmSymbol "begin", ScmPair (ScmNil, ScmNil)), 
                                                                                    ScmNil)))), 
                                                ScmNil))))
     | ScmPair (ScmPair (q, ScmPair (ScmSymbol "=>", body)), rest) ->  (* test => (..) \n test => (..) ... else..*)
        (ScmPair (ScmSymbol "let", ScmPair (
                                       ScmPair (ScmPair (ScmSymbol "value", ScmPair (q, ScmNil)), 
                                                ScmPair (ScmPair (ScmSymbol "f", ScmPair (ScmPair (ScmSymbol "lambda", 
                                                                                                   ScmPair (ScmNil, 
                                                                                                            ScmPair (ScmPair (ScmSymbol "begin", body), 
                                                                                                                     ScmNil))), 
                                       ...
                                       ScmPair (ScmPair (ScmSymbol "if", 
                                                         ScmPair (ScmSymbol "value", 
                                                                  ScmPair (ScmPair (ScmPair (ScmSymbol "f", ScmNil), (* (f) value*)
                                                                                    ScmPair (ScmSymbol "value", ScmNil)), 
                                                                           ScmPair (ScmPair (ScmSymbol "rest", ScmNil), (* (rest)*)
                                                                                    ScmNil)))), 
                                                ScmNil))))
     | ScmPair (ScmPair (q, body), rest) -> (* (zero? n) (f x) (g y)*)
        (ScmPair (ScmSymbol "if", 
                  ScmPair (q, 
                           ScmPair (ScmPair (ScmSymbol "begin", body), 
                                    ScmPair (macro_expand_cond_ribs rest, ScmNil)))))
     | _ -> raise (X_syntax "COND : Variable cannot be a reserved word");;
   ...
   let names_funcs =
     let rec run = function
       | ScmNil -> ([], [])
       | ScmPair (ScmPair (name, ScmPair (func, ScmNil)), ribs) ->
          let (names, funcs) = run ribs in
          (name :: names, func :: funcs)
       | _ -> raise (X_syntax "malformed cond-ribs")
     in run;;
   
   let scheme_pairs_of_list list last =
     List.fold_right
       (fun car cdr -> ScmPair (car, cdr))
       list
       last;;
   
   let macro_expand_letrec_ribs ribs exprs =
     let (names, funcs) = names_funcs ribs in
     let dummies =
       scheme_pairs_of_list
         (List.map
            (fun name ->
              ScmPair (name,
                       ScmPair
     ...
     let sets =
       List.map2
         (fun name func ->
           ScmPair (ScmSymbol "set!", ScmPair (name, ScmPair (func, ScmNil))))
         names funcs in
     let body = scheme_pairs_of_list sets exprs in
     ScmPair (ScmSymbol "let", ScmPair (dummies, body));;    
   ...
   let rec tag_parse sexpr =
        ...
        let params =
          scheme_sexpr_list_of_sexpr_list
            (List.map (function
                 ...
                 | _ -> 
                    raise (X_syntax "invalid lambda-rib structure"))
               ribs) in
        let args =
          scheme_sexpr_list_of_sexpr_list
            (List.map (function
     ...
     | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
        tag_parse (macro_expand_letrec_ribs ribs exprs)
 ...
 module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
   ...
   (* run this first *)
   (* TODO *)
   let rec annotate_lexical_address pe =
     let rec run params env = function
       ...
       | ScmVarSet (Var(name), expr) ->
          ScmVarSet' (tag_lexical_address_for_var name params env,
                      run params env expr)
   
       | ScmIf (test, thn, els) ->
          ScmIf' (run params env test,
                  run params env thn,
                  run params env els)
   
       | ScmSeq exp_list ->
          ScmSeq' (List.map (fun exp -> run params env exp) exp_list)
   
       | ScmVarDef (Var(name), expr) ->
          ScmVarDef' ((tag_lexical_address_for_var name params env),
                      run params env expr)
   
       | ScmOr exp_list ->
          ScmOr' (List.map (fun exp -> run params env exp) exp_list)
   
       | ScmLambda (params', Simple, body) ->
          ScmLambda' (params', Simple,
                      run params' (params :: env) body)
   
       | ScmLambda (params', Opt opt, body) ->
          ScmLambda' (params', Opt opt,
                      run (params' @ [opt]) (params :: env) body)
   
       | ScmApplic (proc, args) ->
          ScmApplic' (run params env proc,
                      List.map (fun exp -> run params env exp) args,
                      Non_Tail_Call) in
     run [] [] pe;;
   
   (* run this second *)
   let rec annotate_tail_calls =
     let rec run tail = function 
       | ((ScmConst' _) as expr')
         | ((ScmVarGet' _) as expr') ->  expr'
   
       | ScmVarDef' (var, body) -> ScmVarDef'(var, run false body)
       | ScmVarSet' (var, body) -> ScmVarSet'(var, run false body)
   
       | ScmIf' (test, caseT, caseF) ->
       ...
       | ScmSeq' exprs ->
          (match list_and_last exprs with
           | None -> ScmConst' ScmVoid
           | Some (exprs, expr) ->
              let exprs' = List.map (fun expr -> run false expr) exprs in
              let expr' = run tail expr in
              ScmSeq' (exprs' @ [expr']))
 
       | ScmOr' exprs ->
          (match list_and_last exprs with
           | None -> ScmConst' (ScmBoolean false)
           | Some (exprs, expr) ->
              let exprs' = List.map (fun expr -> run false expr) exprs in
              let expr' = run tail expr in
              ScmOr' (exprs' @ [expr']))
       ...
       | ScmApplic' (proc, args, _) ->
          ScmApplic' (run false proc,
                      ...
                      if tail then Tail_Call else Non_Tail_Call)
       | ScmBoxSet' (v, expr') -> ScmBoxSet' (v, run false expr')
       | expr' -> expr'
 ...
 module Code_Generation = struct
   ...
   let make_free_vars_table =
     ...
     in fun exprs' -> run 0 (remove_duplicates
                            (collect_free_vars exprs'));;
   ...
   (* I MADE : *)
   let make_lambda_opt_exit_for_more_and_less = 
     make_make_label ".L_lambda_opt_exit_more_and_less";;
   let make_lambda_opt_arity_less =
     make_make_label ".L_lambda_opt_arity_check_less";;
 
   let make_lambda_opt_loop_first = 
     make_make_label ".L_lambda_opt_loop_copy_params_first";;
   
   let make_lambda_opt_loop_second = 
     make_make_label ".L_lambda_opt_loop_copy_params_second";;
   
   let make_lambda_opt_loop_first_end = 
     make_make_label ".L_lambda_opt_loop_copy_params_first_end";;
   
   let make_lambda_opt_loop_second_end = 
   ...
   let code_gen exprs' =
     ...
     let rec run params env = function
       ...
       | ScmVarSet' (Var' (v, Bound (major, minor)), expr') ->
          ...
          ^ (Printf.sprintf
               "\tmov qword [rbx + 8 * %d], rax\t; bound var %s\n" minor v)
          ^ "\tmov rax, sob_void\n"
       ...
       | ScmLambda' (params', Opt opt, body) ->
          ...
          and label_loop_second = make_lambda_opt_loop_second ()
          and label_loop_first = make_lambda_opt_loop_first ()
          and label_loop_first_end =  make_lambda_opt_loop_first_end ()
          and label_loop_second_end =  make_lambda_opt_loop_second_end ()
          ...
          (* HERE IS THE CODE FOR PARAMS == COUNT. *)
   
          ^ (Printf.sprintf "%s:\n" label_arity_ok)
          ^ (Printf.sprintf "\tsub rsp, 8*1\n")
          ^ (Printf.sprintf "\tmov rdi, rsp\n")
          ^ (Printf.sprintf "\t; Copy ret addr\n")
          ^ (Printf.sprintf "\tmov rbx, qword [rdi + 8*1]\n") 
          ^ (Printf.sprintf "\tmov qword [rdi], rbx\n") 
          ^ (Printf.sprintf "\tadd rdi, 8*1\n") 
          ^ (Printf.sprintf "\t; Copy env\n") 
          ^ (Printf.sprintf "\tmov rbx, qword [rdi + 8*1]\n") 
          ^ (Printf.sprintf "\tmov qword [rdi], rbx\n") 
          ^ (Printf.sprintf "\tadd rdi, 8*1\n") 
          ^ (Printf.sprintf "\t;Update the count\n") 
          ^ (Printf.sprintf "\tmov rbx, qword [rdi + 8*1]\n") 
          ^ (Printf.sprintf "\tinc rbx\n") 
          ^ (Printf.sprintf "\tmov qword [rdi], rbx\n")
          ^ (Printf.sprintf "\tadd rdi, 8*1\n")
          ^ (Printf.sprintf "\t; Copy params : \n")
          ...
          ^ (Printf.sprintf "\tlea rdi, [rsp + 8*2 + 8 * rcx]\n")
          ^ (Printf.sprintf "\tmov rdx, rdi\n")
          ^ (Printf.sprintf "\tlea rsi, [rsp + 8*2 + 8*%d]\n" (List.length params'))
          ^ (Printf.sprintf "\tmov rbx, sob_nil\n")
 
          ^ (Printf.sprintf "%s:\n" label_loop)
          ^ (Printf.sprintf "\tcmp rdi, rsi\n")
          ^ (Printf.sprintf "\tje %s\n" label_loop_end)
          ^ (Printf.sprintf "\tpush rdi\n")
          ^ (Printf.sprintf "\tpush rsi\n")
          ^ (Printf.sprintf "\tpush rbx\n")
          ^ (Printf.sprintf "\tpush rdx\n")
          ^ (Printf.sprintf "\tmov rdi, 1 + 8 + 8\n")
          ^ (Printf.sprintf "\tcall malloc\n")
          ^ (Printf.sprintf "\tpop rdx\n")
          ^ (Printf.sprintf "\tpop rbx\n")
          ^ (Printf.sprintf "\tpop rsi\n")
          ^ (Printf.sprintf "\tpop rdi\n")
          ^ (Printf.sprintf "\tmov byte[rax], T_pair\n" )
          ^ (Printf.sprintf "\tmov r8, qword[rdi]\n" )
          ^ (Printf.sprintf "\tmov qword [rax + 1], r8      ; Copy CAR\n" )
          ^ (Printf.sprintf "\tmov qword [rax + 1 + 8], rbx ; Copy CDR\n" )
          ^ (Printf.sprintf "\tmov rbx, rax\n")
          ^ (Printf.sprintf "\tsub rdi, 8*1\n")
          ^ (Printf.sprintf "\tjmp %s\n" label_loop)
          ^ (Printf.sprintf "%s:\n" label_loop_end)
          ^ (Printf.sprintf "\tmov qword [rdx], rbx\n")
          ^ (Printf.sprintf "\tsub rdx , 8*1\n")
          ^ (Printf.sprintf "\tmov rcx, %d\n" (List.length params'))
          ^ (Printf.sprintf "%s:\n" label_loop_first)
          ^ (Printf.sprintf "\tcmp rcx, 0\n")
          ^ (Printf.sprintf "\tje %s\n" label_loop_first_end)
          ^ (Printf.sprintf "\tmov r8, qword [rsi]\n")
          ^ (Printf.sprintf "\tmov qword [rdx], r8\n")
          ^ (Printf.sprintf "\tsub rsi, 8*1\n")
          ^ (Printf.sprintf "\tsub rdx, 8*1\n")
          ^ (Printf.sprintf "\tdec rcx\n")
          ^ (Printf.sprintf "\tjmp %s\n" label_loop_first)
          ^ (Printf.sprintf "%s:\n" label_loop_first_end)
          ^ (Printf.sprintf "\t;Update COUNT\n")
       ...
       | ScmApplic' (proc, args, Non_Tail_Call) ->  (*TODO: SCMAPPLI *)
          (* Printf.printf "\n#DEBUGGING entering non tail application.\n"; *)
          let args_code =
            String.concat ""
              (List.map
                 (fun arg ->
